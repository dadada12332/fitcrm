...
# Zalkins — инструкции для агента (Codex)
...
- После каждой значимой задачи самостоятельно обновлять Obsidian/`knowledge` по `AI_WORKFLOW.md`;
  не ждать отдельного напоминания владельца. Заполнять смысловой итог текущего Daily и запускать
  полный `npm run docs:sync`, а не только отдельные команды синхронизации.
