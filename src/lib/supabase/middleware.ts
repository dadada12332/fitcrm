...
export async function updateSession(request: NextRequest) {
  ...
  // ── Обычная CRM клуба ──
  const publicPaths = ["/", "/about", "/contacts", "/docs", "/blog", "/terms", "/privacy", "/cookies", "/consent", "/data-processing", "/robots.txt", "/sitemap.xml", "/downloads", "/login", "/register", "/forgot-password", "/reset-password", "/auth", "/tg", "/api/health", "/api/auth", "/api/telegram", "/api/integrations", "/api/invite-track", "/api/pay", "/api/cron", "/api/broadcasts", "/accept-invite"]
