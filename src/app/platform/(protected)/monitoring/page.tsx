...
export default async function MonitoringPage() {
  ...
  return (
    <div className="p-6 lg:p-8 max-w-[1200px] mx-auto">
      ...
      <Panel className="px-4 py-3.5 mb-4 flex items-center gap-2" style={{ background: allOk ? "rgba(34,197,94,0.06)" : "rgba(239,68,68,0.06)", borderColor: allOk ? "rgba(34,197,94,0.25)" : "rgba(239,68,68,0.25)" }}>
        {allOk ? <CheckCircle2 className="w-5 h-5" style={{ color: "#22c55e" }} /> : <XCircle className="w-5 h-5" style={{ color: "#ef4444" }} />}
        <span className="text-sm font-semibold text-white">{allOk ? "Все системы работают штатно" : "Обнаружены проблемы"}</span>
      </Panel>

      <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
        {services.map((s) => (
          <Panel key={s.name} className="px-4 py-3.5 flex items-center gap-3">
            <div className="w-10 h-10 rounded-lg flex items-center justify-center shrink-0" style={{ background: s.ok ? "rgba(34,197,94,0.12)" : "rgba(239,68,68,0.12)", color: s.ok ? "#4ade80" : "#f87171" }}>
              {s.icon}
            </div>
            ...
            <div className="text-right shrink-0">
              <span className="inline-flex items-center gap-1.5 text-xs font-medium" style={{ color: s.ok ? "#4ade80" : "#f87171" }}>
                <span className="w-1.5 h-1.5 rounded-full" style={{ background: s.ok ? "#4ade80" : "#f87171" }} />
