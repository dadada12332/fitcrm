  ...
  return (
    ...
    >
      <div
        ...
        style={{ background: "var(--card)", boxShadow: "0 24px 64px rgba(0,0,0,0.2)" }}
        onClick={(e) => e.stopPropagation()}
      >
        {/* Input */}
        <div className="flex items-center gap-3 px-5 py-4" style={{ borderBottom: "1px solid var(--border-subtle)" }}>
          <Search className="w-5 h-5 flex-shrink-0" style={{ color: loading ? "#3b82f6" : "var(--gray-muted)" }} />
          <input
            ref={inputRef}
            value={query}
        ...
        {results.length > 0 ? (
            ...
            {results.map((r) => (
              ...
              >
                ...
                <div className="flex-1 min-w-0">
                  ...
                  {r.phone && <p className="text-xs mt-0.5" style={{ color: "var(--gray-muted)" }}>{r.phone}</p>}
                </div>
                <div className="text-right flex-shrink-0">
                  {r.membershipName && <p className="text-xs" style={{ color: "var(--on-dark-soft)" }}>{r.membershipName}</p>}
          ...
          <div className="py-8 text-center text-sm" style={{ color: "var(--gray-muted)" }}>Клиент не найден</div>
        ) : query.length === 0 ? (
          <div className="py-6 text-center text-xs" style={{ color: "var(--gray-muted)" }}>Введите имя или телефон</div>
        ) : null}
      </div>
...
function NotificationsPanel({ onClose }: { onClose: () => void }) {
  ...
  return (
    <div
      ...
      style={{ background: "var(--card)", border: "1px solid var(--border)", boxShadow: "0 12px 40px rgba(0,0,0,0.14)" }}
    >
      ...
      <div className="max-h-[360px] overflow-y-auto">
        {notifs === null ? (
          <div className="py-8 text-center text-xs" style={{ color: "var(--gray-muted)" }}>Загрузка...</div>
        ) : notifs.length === 0 ? (
          <div className="py-8 text-center text-sm" style={{ color: "var(--gray-muted)" }}>Нет уведомлений</div>
        ) : (
          notifs.map((n) => (
            <Link
              key={n.id}
      ...
      {notifs && notifs.length > 0 && (
        <div className="px-4 py-3" style={{ borderTop: "1px solid var(--border-subtle)" }}>
          <button onClick={onClose} className="text-xs" style={{ color: "var(--gray-muted)" }}>
            Закрыть
          </button>
        </div>
      )}
    ...
    <>
      ...
      >
        <div className="relative flex items-center w-full">
                  ...
                  <>
                    ...
                    <div className="absolute left-0 top-8 z-20 w-52 rounded-xl py-1.5 text-sm"
                      ...
                      <div className="px-2.5 py-1.5 mb-1">
                        <p className="text-xs font-medium" style={{ color: "var(--gray-muted)" }}>Филиалы</p>
                      </div>
                      {branches.map((b) => (
                        <button key={b} className="w-full flex items-center gap-2 px-3 py-2 text-sm rounded-lg mx-1 transition-colors hover:bg-zinc-50 dark:hover:bg-zinc-800"
                          style={{ width: "calc(100% - 8px)", color: "var(--on-dark)" }}>
                      ...
                      <button className="w-full flex items-center gap-2 px-3 py-2 text-sm transition-colors hover:bg-zinc-50 dark:hover:bg-zinc-800"
                        style={{ color: "var(--on-dark-soft)" }} onClick={() => setBranchOpen(false)}>
          ...
          {/* ── Center: tabs ── */}
          <nav className="hidden md:flex items-center gap-0.5 p-1 rounded-md absolute left-1/2 -translate-x-1/2" style={{ background: "var(--card-2)" }}>
          ...
          <div className="flex items-center gap-1 flex-shrink-0 ml-auto">
            ...
            >
              ...
              <span className="hidden sm:block text-xs px-1.5 py-0.5 rounded" style={{ background: "var(--card-2)", color: "var(--gray-muted)", fontSize: 10 }}>⌘K</span>
            </button>
                  ...
                  <div className="absolute right-0 top-11 z-20 w-52 rounded-xl py-2 text-sm"
                    ...
                    {[
                      ...
                      <Link
                        ...
                        className="w-full flex items-center gap-2 px-3 py-2 text-sm transition-colors hover:bg-zinc-50 dark:hover:bg-zinc-800"
                        style={{ color: "var(--on-dark-soft)" }}
                      >
                        <Icon className="w-4 h-4 flex-shrink-0" style={{ color: "var(--on-dark-soft)" }} />
                        {label}
                      </Link>
                    ))}
