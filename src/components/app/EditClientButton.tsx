"use client"

import { useState, useTransition } from "react"
import { toast } from "@/lib/use-action"
import { Pencil, X, ChevronDown, Calendar } from "lucide-react"
import type { ClientProfile } from "@/lib/client-profile"
import { updateClientAction } from "@/app/(app)/clients/actions"
import { MoneyInput } from "./MoneyInput"

/* ─── Phone helpers ─── */

function phoneDigits(v: string) {
  const d = v.replace(/\D/g, "")
  return d.startsWith("998") ? d.slice(3).slice(0, 9) : d.slice(0, 9)
}

function displayPhone(d: string) {
  if (!d) return ""
  if (d.length <= 2) return `+998 (${d}`
  if (d.length <= 5) return `+998 (${d.slice(0, 2)}) ${d.slice(2)}`
  if (d.length <= 7) return `+998 (${d.slice(0, 2)}) ${d.slice(2, 5)}-${d.slice(5)}`
  return `+998 (${d.slice(0, 2)}) ${d.slice(2, 5)}-${d.slice(5, 7)}-${d.slice(7, 9)}`
}

/* ─── Date helpers ─── */

function dateDigits(v: string) {
  return v.replace(/\D/g, "").slice(0, 8)
}

function displayDate(d: string) {
  if (!d) return ""
  if (d.length <= 2) return d
  if (d.length <= 4) return `${d.slice(0, 2)}.${d.slice(2)}`
  return `${d.slice(0, 2)}.${d.slice(2, 4)}.${d.slice(4)}`
}

function rawDateISO(d: string): string {
  if (d.length !== 8) return ""
  return `${d.slice(4, 8)}-${d.slice(2, 4)}-${d.slice(0, 2)}`
}

function isoToDateDig(iso: string | null): string {
  if (!iso) return ""
  const [y, m, d] = iso.slice(0, 10).split("-")
  return `${d}${m}${y}`
}

/* ─── Phone init ─── */

function phoneToDigits(phone: string | null): string {
  if (!phone) return ""
  const d = phone.replace(/\D/g, "")
  return d.startsWith("998") ? d.slice(3).slice(0, 9) : d.slice(0, 9)
}

/* ─── Styles ─── */

const BASE_INPUT = "h-10 w-full rounded-md text-sm outline-none px-3"
const INPUT_STYLE = { background: "var(--card)", border: "1px solid var(--border)", color: "var(--on-dark)" }
const PLACEHOLDER_COLOR = "var(--on-dark-soft)"
const SELECT_CLS = `${BASE_INPUT} appearance-none pr-8`

function Label({ children, required }: { children: string; required?: boolean }) {
  return (
    <div className="flex items-center gap-0.5 text-sm font-medium mb-1.5" style={{ color: "var(--on-dark)" }}>
      {children}
      {required && <span style={{ color: "#ef4444" }}>*</span>}
    </div>
  )
}

function StyledInput(props: React.InputHTMLAttributes<HTMLInputElement>) {
  return <input {...props} className={BASE_INPUT} style={INPUT_STYLE} />
}

function SelectWrapper({ children }: { children: React.ReactNode }) {
  return (
    <div className="relative">
      {children}
      <ChevronDown className="absolute right-3 top-1/2 -translate-y-1/2 w-4 h-4 pointer-events-none" style={{ color: PLACEHOLDER_COLOR }} />
    </div>
  )
}

function SectionTitle({ children }: { children: string }) {
  return <p className="text-base font-medium" style={{ color: "var(--on-dark)" }}>{children}</p>
}

const SOURCE_OPTIONS = [
  { value: "instagram", label: "Instagram" },
  { value: "facebook",  label: "Facebook / VK" },
  { value: "referral",  label: "Рекомендация" },
  { value: "outdoor",   label: "Наружная реклама" },
  { value: "other",     label: "Другое" },
]

const GENDER_OPTIONS = [
  { value: "male",   label: "Мужской" },
  { value: "female", label: "Женский" },
]

/* ─── Main component ─── */

export function EditClientButton({ client }: { client: ClientProfile }) {
  const [open, setOpen] = useState(false)
  const [pending, startTransition] = useTransition()
  const [error, setError] = useState<string | null>(null)

  // Parse full_name → first + last
  const nameParts = client.name.trim().split(/\s+/)
  const initFirst = nameParts[0] ?? ""
  const initLast  = nameParts.slice(1).join(" ")

  const [firstName, setFirstName] = useState(initFirst)
  const [lastName,  setLastName]  = useState(initLast)
  const [phoneDig,  setPhoneDig]  = useState(() => phoneToDigits(client.phone))
  const [dateDig,   setDateDig]   = useState(() => isoToDateDig(client.birthDate))
  const [gender,    setGender]    = useState(client.gender ?? "")
  const [email,     setEmail]     = useState(client.email ?? "")
  const [source,    setSource]    = useState(client.source ?? "")
  const [trainer,   setTrainer]   = useState(client.trainer ?? "")
  const [notes,     setNotes]     = useState(client.notes ?? "")
  const [balance,   setBalance]   = useState(String(client.balance ?? 0))
  const [debt,      setDebt]      = useState(String(client.debt ?? 0))

  function resetToClient() {
    setFirstName(initFirst)
    setLastName(initLast)
    setPhoneDig(phoneToDigits(client.phone))
    setDateDig(isoToDateDig(client.birthDate))
    setGender(client.gender ?? "")
    setEmail(client.email ?? "")
    setSource(client.source ?? "")
    setTrainer(client.trainer ?? "")
    setNotes(client.notes ?? "")
    setBalance(String(client.balance ?? 0))
    setDebt(String(client.debt ?? 0))
    setError(null)
  }

  function close() { setOpen(false); resetToClient() }

  function handleSubmit() {
    if (!firstName.trim()) { setError("Введите имя клиента"); return }

    const fullName = lastName.trim()
      ? `${firstName.trim()} ${lastName.trim()}`
      : firstName.trim()

    startTransition(async () => {
      const res = await updateClientAction(client.id, {
        full_name: fullName,
        phone: phoneDig.length === 9 ? `+998${phoneDig}` : (client.phone ?? null),
        email: email.trim() || null,
        birth_date: rawDateISO(dateDig) || null,
        gender: gender || null,
        source: source || null,
        notes: notes.trim() || null,
        balance: balance !== "" ? Number(balance) : 0,
        debt: debt !== "" ? Number(debt) : 0,
        trainer_name: trainer.trim() || null,
      })

      if (res.error) {
        setError(res.error)
        toast.error(res.error)
      } else {
        close()
        toast.success("Изменения сохранены")
      }
    })
  }

  return (
    <>
      <button
        onClick={() => setOpen(true)}
        className="flex items-center gap-2 h-10 px-4 rounded-md text-sm font-medium transition-colors hover:bg-zinc-50 dark:hover:bg-zinc-800"
        style={{ background: "var(--card)", border: "1px solid var(--border)", color: "var(--on-dark)" }}
      >
        <Pencil className="w-4 h-4" />
        Редактировать
      </button>

      {open && (
        <div className="fixed inset-0 z-40" style={{ background: "rgba(2,6,23,0.4)" }}>
          <div className="absolute inset-0" onClick={close} />

          <div
            className="absolute top-0 right-0 bottom-0 w-full max-w-[560px] flex flex-col"
            style={{ background: "var(--card)", borderLeft: "1px solid var(--border)", boxShadow: "-8px 0 40px rgba(0,0,0,0.1)" }}
          >
            {/* Header */}
            <div className="flex items-center justify-between px-6 py-4 shrink-0" style={{ borderBottom: "1px solid var(--border)" }}>
              <h3 className="text-2xl font-semibold" style={{ color: "var(--on-dark)", letterSpacing: "-0.144px" }}>
                Редактирование клиента
              </h3>
              <button
                onClick={close}
                className="w-8 h-8 flex items-center justify-center rounded-lg transition-colors hover:bg-zinc-100 dark:hover:bg-zinc-800"
                style={{ background: "var(--card-2)", color: "var(--on-dark-soft)" }}
              >
                <X className="w-4 h-4" />
              </button>
            </div>

            {/* Body */}
            <div className="flex-1 overflow-y-auto">
              <div className="flex flex-col gap-6 px-6 py-5">

                {/* ── Личные данные ── */}
                <div className="flex flex-col gap-4">
                  <SectionTitle>Личные данные</SectionTitle>

                  <div>
                    <Label required>Имя</Label>
                    <StyledInput
                      value={firstName}
                      onChange={(e) => setFirstName(e.target.value)}
                      placeholder="Введите имя"
                      autoFocus
                    />
                  </div>

                  <div>
                    <Label>Фамилия</Label>
                    <StyledInput
                      value={lastName}
                      onChange={(e) => setLastName(e.target.value)}
                      placeholder="Введите фамилию"
                    />
                  </div>

                  <div>
                    <Label>Номер телефона</Label>
                    <input
                      className={BASE_INPUT}
                      style={INPUT_STYLE}
                      value={displayPhone(phoneDig)}
                      placeholder="+998 (__) ___-__-__"
                      onChange={(e) => setPhoneDig(phoneDigits(e.target.value))}
                      inputMode="numeric"
                    />
                  </div>

                  <div className="grid grid-cols-2 gap-3">
                    <div>
                      <Label>Дата рождения</Label>
                      <div className="relative">
                        <input
                          className={BASE_INPUT}
                          style={INPUT_STYLE}
                          value={displayDate(dateDig)}
                          placeholder="дд.мм.гггг"
                          onChange={(e) => setDateDig(dateDigits(e.target.value))}
                          inputMode="numeric"
                        />
                        <Calendar className="absolute right-3 top-1/2 -translate-y-1/2 w-4 h-4 pointer-events-none" style={{ color: PLACEHOLDER_COLOR }} />
                      </div>
                    </div>
                    <div>
                      <Label>Пол</Label>
                      <SelectWrapper>
                        <select
                          className={SELECT_CLS}
                          style={{ ...INPUT_STYLE, color: gender ? "var(--on-dark)" : PLACEHOLDER_COLOR }}
                          value={gender}
                          onChange={(e) => setGender(e.target.value)}
                        >
                          <option value="">Не указан</option>
                          {GENDER_OPTIONS.map((o) => (
                            <option key={o.value} value={o.value}>{o.label}</option>
                          ))}
                        </select>
                      </SelectWrapper>
                    </div>
                  </div>

                  <div>
                    <Label>E-mail</Label>
                    <StyledInput
                      type="email"
                      value={email}
                      onChange={(e) => setEmail(e.target.value)}
                      placeholder="example@gmail.com"
                    />
                  </div>
                </div>

                <div style={{ borderTop: "1px solid var(--border-subtle)" }} />

                {/* ── Дополнительная информация ── */}
                <div className="flex flex-col gap-4">
                  <SectionTitle>Дополнительная информация</SectionTitle>

                  <div>
                    <Label>Персональный тренер</Label>
                    <StyledInput
                      value={trainer}
                      onChange={(e) => setTrainer(e.target.value)}
                      placeholder="Имя тренера"
                    />
                  </div>

                  <div>
                    <Label>Источник</Label>
                    <SelectWrapper>
                      <select
                        className={SELECT_CLS}
                        style={{ ...INPUT_STYLE, color: source ? "var(--on-dark)" : PLACEHOLDER_COLOR }}
                        value={source}
                        onChange={(e) => setSource(e.target.value)}
                      >
                        <option value="">Не указан</option>
                        {SOURCE_OPTIONS.map((o) => (
                          <option key={o.value} value={o.value}>{o.label}</option>
                        ))}
                      </select>
                    </SelectWrapper>
                  </div>

                  <div>
                    <Label>Комментарий</Label>
                    <textarea
                      value={notes}
                      onChange={(e) => setNotes(e.target.value)}
                      placeholder="Введите комментарий..."
                      rows={3}
                      className="w-full rounded-md px-3 py-2.5 text-sm outline-none resize-none"
                      style={{ background: "var(--card)", border: "1px solid var(--border)", color: "var(--on-dark)" }}
                    />
                  </div>
                </div>

                <div style={{ borderTop: "1px solid var(--border-subtle)" }} />

                {/* ── Финансы ── */}
                <div className="flex flex-col gap-4">
                  <SectionTitle>Финансы</SectionTitle>

                  <div className="grid grid-cols-2 gap-3">
                    <div>
                      <Label>Баланс</Label>
                      <MoneyInput
                        value={balance}
                        onChange={(n) => setBalance(String(n))}
                        placeholder="0"
                        className={BASE_INPUT}
                        style={INPUT_STYLE}
                      />
                    </div>
                    <div>
                      <Label>Долг</Label>
                      <MoneyInput
                        value={debt}
                        onChange={(n) => setDebt(String(n))}
                        placeholder="0"
                        className={BASE_INPUT}
                        style={{ ...INPUT_STYLE, color: Number(debt) > 0 ? "#dc2626" : "var(--on-dark)" }}
                      />
                    </div>
                  </div>
                </div>

              </div>
            </div>

            {/* Footer */}
            <div className="shrink-0 px-4 py-4 flex gap-3" style={{ borderTop: "1px solid var(--border)" }}>
              {error ? (
                <>
                  <p className="text-sm flex-1 self-center" style={{ color: "#dc2626" }}>{error}</p>
                  <button
                    onClick={() => setError(null)}
                    className="h-11 px-5 rounded-md text-sm font-medium transition-colors hover:bg-zinc-50 dark:hover:bg-zinc-800"
                    style={{ background: "var(--card)", border: "1px solid var(--border)", color: "var(--on-dark)" }}
                  >
                    Назад
                  </button>
                </>
              ) : (
                <>
                  <button
                    onClick={handleSubmit}
                    disabled={pending}
                    className="flex-1 h-11 rounded-md text-sm font-medium text-white transition-opacity hover:opacity-90 disabled:opacity-50"
                    style={{ background: "var(--on-dark)" }}
                  >
                    {pending ? "Сохранение..." : "Сохранить изменения"}
                  </button>
                  <button
                    onClick={close}
                    className="flex-1 h-11 rounded-md text-sm font-medium transition-colors hover:bg-zinc-50 dark:hover:bg-zinc-800"
                    style={{ background: "var(--card)", border: "1px solid var(--border)", color: "var(--on-dark)" }}
                  >
                    Отмена
                  </button>
                </>
              )}
            </div>
          </div>
        </div>
      )}
    </>
  )
}
