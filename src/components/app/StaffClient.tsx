...
function KpiCard({ label, value, sub, icon: Icon, accent }: {
  ...
  return (
    <div className="rounded-xl p-5 flex items-start gap-4" style={{ background: "white", border: "1px solid #e2e8f0" }}>
      ...
      <div className="min-w-0">
        ...
        <p className="text-2xl font-bold leading-none" style={{ color: "#020617" }}>{value}</p>
...
function AddStaffModal({ onClose, onAdded }: { onClose: () => void; onAdded: (row: any) => void }) {
  ...
  const inpStyle = { border: "1.5px solid #e2e8f0", color: "#020617" }

  return (
    <div className="fixed inset-0 z-[200] flex items-center justify-center p-4" style={{ background: "rgba(0,0,0,0.4)" }}>
      <div className="w-full max-w-md rounded-2xl overflow-hidden" style={{ background: "white", boxShadow: "0 20px 60px rgba(0,0,0,0.2)" }}>
        <div className="flex items-center justify-between px-6 py-5" style={{ borderBottom: "1px solid #f1f5f9" }}>
          <h2 className="text-base font-semibold" style={{ color: "#020617" }}>Добавить сотрудника</h2>
          <button onClick={onClose} className="w-8 h-8 flex items-center justify-center rounded-lg hover:bg-slate-50" style={{ color: "#94a3b8" }}>✕</button>
        </div>
        <form onSubmit={handleSubmit} className="px-6 py-5 space-y-4">
          <div className="grid grid-cols-2 gap-3">
            <div className="col-span-2">
              <label className="block text-xs font-medium mb-1" style={{ color: "#64748b" }}>Имя и фамилия</label>
            ...
            <div>
              <label className="block text-xs font-medium mb-1" style={{ color: "#64748b" }}>Телефон</label>
          ...
          <div>
            <label className="block text-xs font-medium mb-1.5" style={{ color: "#64748b" }}>Роль</label>
          ...
          <div>
            <label className="block text-xs font-medium mb-1.5" style={{ color: "#64748b" }}>Тип зарплаты</label>
          ...
          <div className="grid grid-cols-2 gap-3">
            {(salType === "fixed" || salType === "mixed") && (
              <div className={salType === "fixed" ? "col-span-2" : ""}>
                <label className="block text-xs font-medium mb-1" style={{ color: "#64748b" }}>Фикс (сум)</label>
            ...
            {(salType === "percent" || salType === "mixed") && (
              <div className={salType === "percent" ? "col-span-2" : ""}>
                <label className="block text-xs font-medium mb-1" style={{ color: "#64748b" }}>Процент (%)</label>
          ...
          <div className="flex gap-3 pt-2">
            <button type="button" onClick={onClose}
              className="flex-1 h-10 rounded-xl text-sm font-medium transition-colors hover:bg-slate-50"
    ...
    <>
        ...
        <div className="flex items-center gap-1 p-1 rounded-lg" style={{ background: "#f1f5f9" }}>
          {ROLE_FILTERS.map((f) => (
            <button key={f.key} onClick={() => setRoleFilter(f.key)}
              ...
              style={{
                background: roleFilter === f.key ? "white" : "transparent",
                color: roleFilter === f.key ? "#020617" : "#64748b",
      ...
      <div className="rounded-xl overflow-hidden" style={{ background: "white", border: "1px solid #e2e8f0" }}>
        <div className="px-5 py-4" style={{ borderBottom: "1px solid #f1f5f9" }}>
          <p className="text-sm font-semibold" style={{ color: "#020617" }}>{filtered.length} сотрудников</p>
        </div>

        {filtered.length === 0 ? (
          <div className="py-16 text-center text-sm" style={{ color: "#94a3b8" }}>Сотрудников нет</div>
        ) : (
              ...
              <thead>
                <tr style={{ borderBottom: "1px solid #f1f5f9" }}>
                  {["Сотрудник","Роль","Клиенты","Зарплата","Статус",""].map((h) => (
                    <th key={h} className="px-5 py-3 text-left text-xs font-medium" style={{ color: "#94a3b8" }}>{h}</th>
                  ))}
                </tr>
              </thead>
              <tbody>
                    ...
                    <tr key={row.id} className="transition-colors hover:bg-slate-50" style={{ borderBottom: "1px solid #f8fafc" }}>
                      <td className="px-5 py-3">
                        <div className="flex items-center gap-3">
                          ...
                          <div className="min-w-0">
                            <p className="font-medium truncate" style={{ color: "#020617" }}>{row.name}</p>
                            <p className="text-xs truncate" style={{ color: "#94a3b8" }}>{row.email}</p>
                          </div>
                        </div>
                      </td>
                      <td className="px-5 py-3">
                      ...
                      <td className="px-5 py-3 font-medium" style={{ color: "#020617" }}>
