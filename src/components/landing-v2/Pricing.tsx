    ...
    <section id="pricing" className="py-24 px-4">
      <div className="max-w-5xl mx-auto">
        <Reveal className="text-center mb-10">
          ...
          <div className="inline-flex items-center gap-1 mt-8 p-1 rounded-full bg-white border border-zinc-200">
            {[{ k: false, l: "Месяц" }, { k: true, l: "Год −20%" }].map((o) => (
              <button key={String(o.k)} onClick={() => setYearly(o.k)}
                ...
                style={{ background: yearly === o.k ? "#2563eb" : "transparent", color: yearly === o.k ? "#fff" : "#71717a" }}>
                {o.l}
              </button>
            ))}
          </div>
          ...
          {plans.map((p, i) => {
            ...
            return (
              <motion.div key={p.name}
                ...
                className={`relative rounded-2xl p-7 flex flex-col ${p.hl ? "" : "v2-card"}`}
                style={p.hl ? { background: "#2563eb", boxShadow: "0 40px 80px -30px rgba(37,99,235,0.6)" } : undefined}
              >
                <div className={`text-sm ${p.hl ? "text-white/85" : "v2-muted"}`}>{p.name}</div>
                <div className="mt-1 flex items-baseline gap-1.5">
                  <span className={`text-3xl font-bold ${p.hl ? "text-white" : "text-zinc-900"}`}>{fmt(price)}</span>
                  <span className={`text-xs ${p.hl ? "text-white/75" : "v2-muted"}`}>сум / мес</span>
                </div>
                <ul className="flex flex-col gap-2.5 flex-1 mt-6">
                  {p.features.map((f) => (
                    <li key={f} className="flex items-center gap-2.5 text-sm">
                      <Check className="w-4 h-4 flex-shrink-0" style={{ color: p.hl ? "#fff" : "#2563eb" }} />
                      <span className={p.hl ? "text-white/90" : "text-zinc-700"}>{f}</span>
                    </li>
                  ))}
                </ul>
                <Link href={p.href}
                  className={`mt-6 inline-flex items-center justify-center h-11 rounded-full text-sm font-medium transition-transform hover:scale-[1.01] ${p.hl ? "" : "v2-btn-primary"}`}
                  style={p.hl ? { background: "#fff", color: "#2563eb" } : undefined}>
                  {p.cta}
                </Link>
              </motion.div>
            )
