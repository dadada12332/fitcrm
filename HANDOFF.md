# Zalkins — полная передача проекта (Handoff для нового AI/CTO)

> Этот документ написан как передача дел от инженера, который строил Zalkins практически с нуля.
...
- **Zalkins** — мультитенантный SaaS CRM для фитнес-клубов. Рынок — Узбекистан (валюта UZS/сум,
...
## 1. Что такое Zalkins (продукт и бизнес)

Zalkins — это операционная система для управления фитнес-клубом. Один клуб (tenant) внутри
...
| Telegram-бот | **grammy** | У каждого клуба СВОЙ бот (`telegram_integrations.bot_token`, service-only). Плюс есть общий/системный бот из env. |
...
- `TELEGRAM_BOT_TOKEN`, `TELEGRAM_CRM_BOT_TOKEN`, `TELEGRAM_CHAT_ID`, `TELEGRAM_WEBHOOK_SECRET`,
  `TELEGRAM_SETUP_SECRET` — Telegram. Токены клубных ботов хранятся service-only в `telegram_integrations`.
...
- **У каждого клуба СВОЙ бот** — токен в service-only `telegram_integrations` (подключается на `/integrations` →
  Основное). Connect action регистрирует защищённый dynamic webhook
  `/api/telegram/webhook/[clubId]`; legacy общий webhook отключён.
- **Вся логика бота** — `src/lib/telegram/bot.ts` (grammy): команды `/start`, привязка клиента
  (только собственный Telegram contact и только внутри route `clubId`), привязка сотрудника,
  абонемент, расписание, QR, reminders и Payme/Click self-service renewal.
- **`telegram_users`** — club-scoped связь с unique `(club_id, telegram_id)`; один человек может
  быть клиентом нескольких клубных ботов. `telegram_events` — delivery/activity audit и метрики.
- **Рассылка** (`TelegramBroadcast` + `broadcast.ts`): получатели = клиенты с `telegram_id`
  ...
  Supabase Cron каждые 5 минут (`scripts/configure-broadcast-scheduler.mjs`), secret в Vault.
- **Автоматизация** `/api/telegram/reminders/run`: ежедневно отправляет настроенные expiry reminders
  и список забронированных занятий; `telegram_events.idempotency_key` защищает от дублей.
- **QR-чекин:** бот показывает QR из `clients.qr_token`, CRM сканирует его камерой на `/visits`.
- **Настоящего auto-charge пока нет:** текущие payment links не дают recurring card mandate;
  бот делает безопасное ручное продление, активируемое только после provider callback.
- Подробный runbook и продуктовая модель: `knowledge/Integrations/Telegram.md`.
- **Крон дневного отчёта** `api/telegram/daily-report`: **ИСПРАВЛЕНА утечка мультитенантности.**
  ...
  `staff.role`), через service-only токен бота этого клуба. Глобальный чат больше не используется.
