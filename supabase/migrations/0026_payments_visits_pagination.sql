-- 0026_payments_visits_pagination.sql
-- Серверная пагинация разделов «Оплаты» и «Посещения».
-- Границы периода/дат вычисляются в приложении и передаются как timestamptz.
-- Идемпотентно.
...
-- ── Оплаты: фильтр(период/провайдер/статус/поиск) + сортировка + пагинация + count + сумма ──
create or replace function public.payments_page(
  p_club_id  uuid,
  p_search   text        default null,
  p_from     timestamptz default null,   -- нижняя граница периода (coalesce(paid_at,created_at) >= p_from)
  p_provider text        default null,   -- cash|click|payme|uzum
  p_status   text        default null,   -- paid|pending|failed|refunded
  p_sort     text        default null,   -- amount_desc|amount_asc|created_asc|(default created_desc)
...
language sql stable security definer set search_path = public as $$
  with base as (
    select
      p.id, p.client_id, cl.full_name as client_name, cl.phone as client_phone,
    ...
    where p.club_id = p_club_id
      ...
      and (p_search   is null or p_search   = ''
           or cl.full_name ilike '%' || p_search || '%'
           or m.name       ilike '%' || p_search || '%')
  )
  select
    b.id, b.client_id, b.client_name, b.client_phone, b.service_name,
    b.amount, b.provider, b.status, b.paid_at, b.created_at,
    count(*) over () as total_count,
    coalesce(sum(b.amount) over (), 0) as total_amount
  from base b
  order by
...
-- ── Посещения: журнал за период(по умолчанию сегодня) + статус-фильтр + пагинация + count ──
create or replace function public.visits_page(
  p_club_id uuid,
  p_search  text        default null,
  p_from    timestamptz default null,
  p_to      timestamptz default null,
  p_status  text        default null,   -- all|active|ending|expired
  p_sort    text        default null,   -- checked_asc|(default checked_desc)
  p_limit   int         default 50,
  p_offset  int         default 0
)
returns table (
...
language sql stable security definer set search_path = public as $$
  with base as (
    select
      v.id, v.client_id, cl.full_name as client_name, cl.phone as client_phone, v.checked_in_at,
  ...
  select
    id, client_id, client_name, client_phone, checked_in_at, membership_name, sub_status,
