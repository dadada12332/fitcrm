...
-- Серверная пагинация раздела «Клиенты». Идемпотентно.
-- ============================================================
...
create or replace function public.clients_page(
  ...
  p_statuses text[] default null, p_memberships text[] default null, p_days text[] default null,
...
language sql stable security definer set search_path = public as $$
  with base as (
    select
      ...
      case when c.debt is not null then c.debt else greatest(-coalesce(c.balance, 0), 0) end as debt,
    ...
    left join lateral (
      ...
      order by (case sub.status when 'active' then 0 when 'frozen' then 1 else 2 end), sub.expires_at desc nulls last
      limit 1
    ) s on true
    left join lateral (
  ...
  derived as (
    select id, name, phone, birth_date, gender, source, created_at, debt, membership,
      expires_at, visits_total, visits_used, last_visit,
      coalesce(sub_status, 'none') as status,
      case when sub_status in ('active','frozen') and expires_at is not null then (expires_at - current_date)::int end as days_left,
      case when visits_total is not null and visits_used is not null then greatest(0, visits_total - visits_used) end as visits_left
    from base
  ),
  filtered as (
    select * from derived
    where
      (p_search is null or p_search = '' or name ilike '%' || p_search || '%' or coalesce(phone, '') ilike '%' || p_search || '%')
  ...
  select f.id, f.name, f.phone, f.birth_date, f.gender, f.source, f.membership,
...
language sql stable security definer set search_path = public as $$
  select
    ...
    (select count(distinct client_id) from public.subscriptions where club_id = p_club_id and status = 'active'),
    (select count(distinct client_id) from public.subscriptions where club_id = p_club_id and status = 'active' and expires_at is not null and expires_at <= current_date + 7),
    (select coalesce(sum(case when debt is not null then debt else greatest(-coalesce(balance,0),0) end), 0) from public.clients where club_id = p_club_id);
$$;
