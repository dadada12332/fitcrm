...
| preview | Vercel-generated URL | Основной Supabase: `ap-southeast-2`; отдельный preview DB не настроен | `syd1` по `vercel.json` | Проверка веток/deploy | Vercel preview | Не проверено |
...
| production | `https://fitcrm-three.vercel.app` | Project ID не фиксируется в Vault; регион `ap-southeast-2` (Sydney), подтверждено 2026-07-18 | `syd1` (Sydney) | Рабочая CRM | push `main`, затем alias при необходимости | Supabase, Telegram, Payme, Click; SMS/email не подтверждены |
