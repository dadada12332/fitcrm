...
- В репозитории последовательные миграции `0001`–`0085` и отдельный timestamped index; `0085` расширяет service-only OAuth/sync-контур для Google Calendar.
...
- Google Calendar OAuth ожидает Google Cloud Client ID/Secret и включения Calendar API; без них подключение в UI намеренно недоступно.
