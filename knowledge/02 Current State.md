...
- `20260805075601_lead_hub_foundation.sql` применена: 10 tenant-scoped таблиц Lead Hub,
  11 service-only RPC, immutable history/conversion, явные Data API grants, RLS и индексы.
- `20260807115921_lead_hub_client_integrity.sql` применена: conversion client обязателен,
  обе tenant FK блокируют удаление клиента из immutable sales trail через `ON DELETE RESTRICT`.
