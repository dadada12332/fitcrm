...
Экран «Помощь клуба» создаёт обращения в `client_conversations` и сообщения в `client_conversation_messages`. Клиент определяется только по ранее подтверждённому `client_id`; Telegram имя служит подписью. Ответы сотрудников сохраняются до отправки, доставляются тем же ботом клуба и повторяются Supabase Cron через `/api/telegram/client-support/run` при временной ошибке. Решённый диалог можно переоткрыть ответом или закрыть в историю при явном создании нового вопроса.
